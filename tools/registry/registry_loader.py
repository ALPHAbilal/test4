...
def initialize_tool_registry(config_path: Optional[str] = None) -> ToolRegistry:
    ...
    if config_path is None:
        ...
        config_path = os.path.join(os.path.dirname(current_dir), 'config', 'tool_definitions_fixed.json')

        # If the fixed file doesn't exist, fall back to the original
        if not os.path.exists(config_path):
            logger.warning(f"Fixed tool definitions file not found: {config_path}, falling back to original")
